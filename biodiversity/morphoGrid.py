...
class morphoGrid:
    ...
    def loadHDF5(self, timestep=0, basinIDs=None, XYf=None):
        """
        ...
        if basinIDs is not None:
            base= basinIDs
            tree2 = cKDTree(XYf)
            distancesf, indicesf = tree2.query(xyi, k=3)
            b_vals = base[indicesf][:,:,0]
            onIDs = np.where(distancesf[:,0] > 0)[0]
        ...
        #bi[bi<1] = 0.
        self.catchment = np.reshape(bi,(self.ny,self.nx)).astype(int)
